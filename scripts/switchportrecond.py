...
# dnsmasq's lease file. Sites still on ISC dhcpd (lavergne) override this
# with --leases-path; lookup_lease_ip auto-detects the on-disk format.
...
def render_status_snapshot(ps, deepest_state):
    ...
    return {
        ...
        # on/off/unknown, independent of the wattage and of the composite
        # `state` (which can sit past bmcpower). The Triage UI takes the
        # power-pill COLOUR from this and the watts LABEL from `power` --
        # a host draws standby watts when off, so watts alone can't tell
        # on from off. Mirrors flax_control/triage_compat.py.
        "bmcpower": (ps["vars"].get("bmcpower") or {}).get("value") or "unknown",
        # Platform identity from the BMC FRU probe. The bmc_fw worker gates
        # is_candidate() on product_name and matches the manifest's
        # product_name regex against it, so a snapshot without these two
        # yields zero candidates and no FW status in the UI. Same field names
        # and "unknown" fallback as flax_control/triage_compat.py.
        "product_name": (ps.get("bmc_kind_cached") or {}).get("product_name") or "unknown",
        "bmc_kind": (ps.get("bmc_kind_cached") or {}).get("kind") or "unknown",
...
def _parse_watts_from_ipmi_output(text):
    ...
    Two spellings in the field: 'HSC Input Power' (the name this was
    written against) and 'MB HSC In Power' (lavergne's Quanta TiogaPass).
    Match both — a miss here silently yields no wattage rather than an
    error, which is how the blank power readings at lavergne went
    unnoticed.
    """
    for line in text.splitlines():
        low = line.lower()
        if "hsc input power" in low or "hsc in power" in low:
            parts = line.split("|")
            if len(parts) >= 2:
...
def bmc_input_power_traditional(ip, creds_pair):
    ...
    Same two spellings as _parse_watts_from_ipmi_output — keep in step.
    """
    try:
...
def lookup_isc_lease_ip(leases_path, mac):
    """Get the IP for a MAC from an ISC dhcpd.leases file.

    lavergne predates reaper and serves DHCP with ISC dhcpd rather than
    dnsmasq, so its lease file is stanza-shaped instead of line-shaped:

        lease 192.168.0.242 {
          binding state active;
          hardware ethernet 0c:48:c6:bc:9c:f0;
        }

    ISC appends rather than rewrites, so later stanzas supersede earlier
    ones for the same address — scan the whole file and keep the LAST
    match rather than returning on the first.

    An 'active' binding always wins over a non-active one: a freed lease
    still carries its hardware line, and a BMC pinned locally to its
    reservation address can linger as 'free' while still answering there.
    Falling back to the most recent non-active match mirrors what the
    dnsmasq dhcp-hosts fallback does for statically-pinned BMCs.
    ...
    try:
        with open(leases_path) as f:
            for line in f:
                s = line.strip()
                if s.startswith("lease ") and s.endswith("{"):
                    parts = s.split()
                    cur_ip = parts[1] if len(parts) >= 2 else None
                    cur_mac = None
                    cur_active = False
                elif s.startswith("hardware ethernet"):
                    cur_mac = s.split()[-1].rstrip(";").lower()
                elif s.startswith("binding state"):
                    cur_active = s.rstrip(";").split()[-1] == "active"
                elif s.startswith("}"):
                    if cur_ip and cur_mac == target:
                        best_any = cur_ip
                        if cur_active:
                            best_active = cur_ip
                    cur_ip = None
        ...
        return None
    return best_active or best_any
...
def lookup_lease_ip(leases_path, mac, dhcp_hosts_dir=None):
    ...
    try:
        ...
        pass
    # ISC dhcpd fallback (lavergne). Safe to try unconditionally against the
    # same path: the dnsmasq scan above cannot false-positive on an ISC file
    # because 'hardware ethernet <mac>;' puts 'ethernet' in the MAC position,
    # and the ISC parser returns None on a dnsmasq-shaped file for the
    # mirror-image reason (no 'lease <ip> {' stanzas to open).
    isc_ip = lookup_isc_lease_ip(leases_path, mac)
    if isc_ip:
        return isc_ip
    if dhcp_hosts_dir:
        try:
...
def port_worker_one_iter(port_state, switch_facts, emit_event, env):
    ...
    if not bmc_ip:
        ...
        needs_reprobe = (
            ...
            # product_name was added to the cache after some snapshots were
            # already on disk; a cache without the key predates it, so re-probe
            # once to fill it rather than emitting null forever.
            or "product_name" not in cache
        )
        if needs_reprobe:
            probe = probe_bmc_kind(bmc_ip, credentials, bmc_creds)
            cache = {"kind": probe["kind"],
                     # Kept for the status snapshot: the bmc_fw worker's
                     # is_candidate() gate requires product_name, and the
                     # manifest matches platforms on it.
...
def main(argv=None):
    ...
    # Sites on ISC dhcpd (lavergne) point this at /var/lib/dhcp/db/dhcpd.leases;
    # the reader auto-detects stanza vs line format, so only the path differs.
    parser.add_argument("--leases-path", default=LEASES_PATH)
