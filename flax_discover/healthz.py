"""Tiny healthz server for flax-discover. Mirrors flax_classify.healthz.

flax-discover's cycle returns {"written": N, "superseded": M}, so
record_cycle_done takes `written` plus an optional `superseded` (defaults to 0
for older callers). Otherwise identical shape: status starting -> ok -> stale +
age_secs, time.monotonic internally.
"""
import json
import threading
import time
...
class HealthState:
    ...
    def record_cycle_done(self, *, written: int, superseded: int = 0,
                          swept: int = 0):
        with self._lock:
            self._last_cycle_ts = time.monotonic()
            self._last_cycle = {"written": written, "superseded": superseded,
                                "swept": swept}
